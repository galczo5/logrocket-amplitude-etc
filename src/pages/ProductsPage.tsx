...
export default function ProductsPage() {
  ...
  const fetchProducts = useCallback((f: FilterValues, s: string, q: string) => {
    ...
    api
      ...
      .then(({ products: raw }) => {
        ...
        if (f.sizes.length > 1)
          result = result.filter((p) => f.sizes.some((sz) => p.sizes.includes(sz)));
  ...
  function removeFilter(type: string, value?: string) {
    if (type === "category")
      ...
      setFilters((f) => ({ ...f, sizes: f.sizes.filter((s) => s !== value) }));
    else if (type === "color")
      setFilters((f) => ({ ...f, colors: f.colors.filter((c) => c !== value) }));
    else if (type === "price")
          ...
          <div className="flex flex-wrap gap-2 items-center">
            {activeBadges.map((b) => (
              <Badge
                key={`${b.type}-${b.value ?? ""}`}
                variant="secondary"
                className="gap-1 capitalize pl-2 pr-1"
              >
                {b.label}
                <button
